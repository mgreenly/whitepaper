...
**Inter-Scope Communication**: Actions can only access the current request's input data via `.current.*` because user-supplied names are dynamic and unknown at template write-time. To share data between catalog items or bundle components, actions must publish outputs that can be consumed by later actions using known output keys.

**Key Design Principle**: Since catalog templates are written at design-time but user-supplied names are provided at runtime, templates cannot reference other requests' input data. All cross-request data sharing must flow through the output mechanism where the consuming template knows the specific user-provided name to reference (e.g., `{{.output.webapp.databaseUrl}}`).
...
{{.current.config.instanceName}}               # From config group (current request)
{{.current.config.secretName}}                 # From config group (current request)
```
...
- **CatalogItem Fulfillment**: Can reference `.current.*`, `.output.*`, `.system.*`, and all `.metadata.*` (all catalog item metadata is always available)
...
3. **Data Population**: Form data populates the `.current` namespace with structure `{{.current.groupId.fieldId}}`
4. **Component 1**: Accesses its required input fields and computes outputs, populates `.output.*` namespace
5. **Component 2**: Can reference previous `.output.*` values plus its required `.current.*` fields and `.metadata.*`
...
# .current.config.instanceName = "webapp-db"        # From database component (config group)
# .current.config.instanceClass = "db.t3.medium"    # From database component (config group)
# .current.config.secretName = "webapp-secrets"     # From secrets component (config group)  
# .current.basic.appName = "myapp"                  # From application component (basic group)
# .current.basic.containerImage = "nginx:latest"    # From application component (basic group)
...
# Template uses: {{.current.config.instanceName}}, {{.current.config.instanceClass}}
# Computes and adds to .output:
# .output.webapp.databaseUrl = "postgres://{{.current.config.instanceName}}.aws.com:5432/db"
# .output.webapp.databaseName = "{{.current.config.instanceName}}"
...
# Template uses: {{.current.config.secretName}} and previous outputs
# Can reference: {{.output.webapp.databaseUrl}}, {{.output.webapp.appUrl}}, {{.current.config.secretName}}

# Key Point: All components share the same .current namespace since it's the same request.
# Inter-component data sharing happens through .output variables with known user-supplied names.
```
