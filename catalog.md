...
**Dynamic Form Generation**: Bundle input forms are automatically generated by merging input requirements from all referenced CatalogItems. This form generation is handled externally by the orchestrator service and is not a concern of the catalog definition. The catalog simply defines the structure and relationships between components.
...
# Note: Bundle input forms are automatically generated by merging
# the input forms from all referenced CatalogItems (database-aurora-postgresql-standard,
# security-parameterstore-standard, compute-eks-containerapp).
# No manual input form definition is required.
...
|-----------|---------|-------------------|----------------|
| `.input` | Raw user input form data | Request submission | `{{.input.NAME.componentId.groupId.fieldId}}` (bundles) or `{{.input.NAME.groupId.fieldId}}` (individual items) |
| `.output` | Computed values during fulfillment | After each action (sequential) | `{{.output.NAME.computedValue}}` |
...
2. **Output Processing**: After successful execution, any outputs generated by the action are added to the `.output.NAME.*` namespace
...
```
{{.input.myapp.application.basic.appName}}      # From application component's basic group (user name: myapp)
{{.input.webapp.database.config.instanceName}}  # From database component's config group (user name: webapp)  
{{.input.mydb.secrets.config.secretName}}       # From secrets component's config group (user name: mydb)
```
...
- **CatalogItem Fulfillment**: Can reference `.input.{name}.{componentId}.*`, `.output.{name}.*`, `.system.*`, and all `.metadata.*` (all catalog item metadata is always available). When used individually, componentId is omitted: `.input.{name}.*`
...
- **Action Templates**: Can reference all variable namespaces:
  - `.input.{name}.{componentId}.*` - User form data scoped under user-supplied name and component ID (individual items omit componentId)
...
2. **Input/Output Variable Namespacing**: Creates the `.input.{name}.*` and `.output.{name}.*` namespaces where user form data and computed fulfillment outputs are stored. For bundles, input data is further namespaced by component ID: `.input.{name}.{componentId}.*`. This `{name}` becomes the dynamic namespace key that scopes all user-specific data.

## Bundle-to-Component Data Flow

When a bundle is submitted, the orchestrator automatically merges input requirements from all referenced components to create the bundle's dynamic input form. Users fill out this merged form, and the data becomes available to all component fulfillment templates through the `.input` namespace. The `.output` namespace accumulates computed values as each component is processed sequentially.

**Data Flow Process**:
1. **Form Generation**: Orchestrator merges input forms from all referenced CatalogItems to create dynamic bundle form
2. **User Input**: User fills out the dynamically generated form containing fields from all components
3. **Data Population**: Form data populates the `.input` namespace with structure `{{.input.NAME.componentId.groupId.fieldId}}`
4. **Component 1**: Accesses its required input fields and computes outputs, populates `.output.*` namespace
5. **Component 2**: Can reference previous `.output.*` values plus its required `.input.*` fields and `.metadata.*`
6. **Sequential Processing**: Each component builds upon previous computed outputs while accessing its own input fields

**Dynamic Form Inheritance**: Bundles automatically inherit and merge input forms from all referenced components. This form inheritance is handled externally by the orchestrator service.

**Example Data Flow**:
```yaml
# Bundle input form automatically merges fields from all components (using required 'name' field = "webapp"):
# .input.webapp.database.config.instanceName = "webapp-db"
# .input.webapp.database.config.instanceClass = "db.t3.medium"
# .input.webapp.secrets.config.secretName = "webapp-secrets"
# .input.webapp.application.basic.appName = "myapp"
# .input.webapp.application.basic.containerImage = "nginx:latest"
...
# Template uses: {{.input.webapp.database.config.instanceName}}, {{.input.webapp.database.config.instanceClass}}
# Computes and adds to .output:
# .output.webapp.databaseUrl = "postgres://{{.input.webapp.database.config.instanceName}}.aws.com:5432/db"
# .output.webapp.databaseName = "{{.input.webapp.database.config.instanceName}}"
...
# Template uses: {{.output.webapp.databaseUrl}} and {{.input.webapp.application.basic.appName}}
# Computes: .output.webapp.appUrl = "https://{{.input.webapp.application.basic.appName}}.company.com"
...
# Template uses: {{.input.webapp.secrets.config.secretName}} and previous outputs
# Can reference: {{.output.webapp.databaseUrl}}, {{.output.webapp.appUrl}}, {{.input.webapp.secrets.*}}
```
...
input:
  form:
    groups:
# Bundle input form is automatically generated by merging input forms from:
# 1. database-aurora-postgresql-standard (adds database.config group with instanceName, instanceClass, storageSize fields)
# 2. security-parameterstore-standard (adds secrets.config group with secretName, secrets fields)  
# 3. compute-eks-containerapp (adds application.basic group with appName, containerImage, replicas fields)
# Plus the required 'name' field for the bundle resource identifier
