...
| `select` | Single choice | enum (field-level) | Dropdown |
| `multiselect` | Multiple choices | enum (field-level) | Multi-select |
...
**Variable Scoping Responsibilities**: With the global `.output.*` namespace, variable shadowing can occur when multiple actions use the same output key names. This is accepted as a design tradeoff for simplicity. Template writers should choose descriptive, unique key names to minimize conflicts. Future validation enhancements may detect and report potential shadowing issues to help authors avoid unintended overwrites.
...
| `.output` | Computed values during fulfillment | After each action (sequential) | `{{.output.userSelectedKey}}` |
...
2. **Output Processing**: After successful execution, any outputs generated by the action are added to the `.output.*` namespace using user-selected keys
...
**Inter-Scope Communication**: Actions can access the current request's input data via `.current.*` and can reference outputs from previous actions via `.output.*` using globally scoped keys.

**Key Design Principle**: Templates are processed at runtime with a global output namespace. Actions populate `.output.*` with user-selected keys (e.g., `{{.output.databaseUrl}}`, `{{.output.secretArn}}`). Variable name shadowing is possible and accepted as a design tradeoff for simplicity.
...
The `.output` namespace is completely flat with no nested structure. Fulfillment actions store computed values using user-selected keys directly in the top-level output space:

```
.output
├── databaseUrl                   # User-selected key, value from action
├── secretArn                     # User-selected key, value from action  
├── applicationUrl                # User-selected key, value from action
└── resourceId                    # User-selected key, value from action
```

**Examples for Different Scenarios**:

**Individual CatalogItem**:
```
.output
├── databaseUrl                   # User-selected key from database action
├── resourceTags                  # User-selected key from tagging action
└── connectionString              # User-selected key from connection action
```

**Bundle Components**:
```
.output
├── databaseEndpoint              # User-selected key from database component
├── secretsArn                    # User-selected key from secrets component
└── applicationUrl                # User-selected key from application component
```
...
```
{{.output.databaseUrl}}                 # Computed connection string
{{.output.resourceTags}}                # Computed resource tags
{{.output.secretArn}}                   # ARN from previous action
{{.output.applicationUrl}}              # Application endpoint URL
```
...
**Structure**: `{{.output.userSelectedKey}}`
- `{userSelectedKey}`: Key chosen by action authors, stored directly in flat output namespace

**Population**: Each action can add key-value pairs directly to the flat output namespace

**Examples**:
```
{{.output.databaseUrl}}                 # string - Database connection URL
{{.output.databaseName}}                # string - Created database name
{{.output.resourceId}}                  # string - AWS resource identifier
{{.output.secretArn}}                   # string - AWS Parameter Store ARN
{{.output.applicationUrl}}              # string - Application endpoint URL
```
...
- **Action Templates**: Can reference all variable namespaces:
  ...
  - `.output.*` - Computed values from previous actions in global namespace
...
**Name Field Usage**: The user-supplied `name` field serves as **Resource Identification**, becoming the basis for AWS resource names, Kubernetes namespaces, etc. The `.output.*` namespace uses a global scope with user-selected keys rather than being scoped by the name field.
...
**Component Template Access**: When bundle components execute, each component's CatalogItem fulfillment templates have access to the entire bundle form data through the `.current` namespace. This means component templates can reference any field from the bundle's input form, not just fields specific to that component.
...
# .output.databaseName = "{{.current.basic.instanceName}}"
...
# Template uses: {{.output.databaseUrl}} and {{.current.basic.appName}}
...
# Inter-component data sharing happens through .output variables in a global namespace.
```
